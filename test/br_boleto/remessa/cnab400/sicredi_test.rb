...
describe BrBoleto::Remessa::Cnab400::Sicredi do
	...
	describe "#detalhe_posicao_048_062(pagamento)" do
		it "deve ter o tamanho de 15 digitos" do
			subject.detalhe_posicao_048_062(pagamento).size.must_equal 15
		end
		it "deve conter as informações nas posições corretas" do
			pagamento.assign_attributes(nosso_numero: '132')
			result = subject.detalhe_posicao_048_062(pagamento)
			result[0..8].must_equal    '000000132'    # Nosso Numero
			result[9..14].must_equal   ''.rjust(6)    # Brancos
			result.size.must_equal 15
		end
	end
